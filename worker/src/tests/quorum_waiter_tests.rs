...
use super::*;
use crate::common::{batch, committee_with_base_port, keys, listener, TestTxType};
...
async fn wait_for_quorum() {
    ...
    let (myself, _) = keys(0).pop().unwrap();
    let committee = committee_with_base_port(7_000, 0);

    // Spawn a `QuorumWaiter` instance.
    QuorumWaiter::spawn(committee.clone(), /* stake */ 1, rx_message, tx_batch);

    // Make a batch.
    let message = WorkerMessage::Batch(batch(TestTxType::TxForCreateTemAcc));
