...
use crate::worker::{WorkerMessage, ConversionMessage};
use bytes::Bytes;
...
use primary::{PrimaryWorkerMessage, Round};
...
pub struct Synchronizer {
    ...
    // 
    tx_process: Sender<ConversionMessage>,
}
...
impl Synchronizer {
    ...
    async fn waiter(
        ...
        tokio::select! {
            ...
            _ = handler.recv() => Ok(None), // for garbage collection
        }
    }
        ...
        loop {
            tokio::select! {
                ...
                Some(message) = self.rx_message.recv() => match message {
                    ...
                    PrimaryWorkerMessage::Cleanup(height, header) => {
                        // Keep track of the primary's leading round number.
                        if header.round() > self.round {
                          self.round = header.round();
                        }

                        debug!(
                          "Receiving cleanup msg for: {}, height: {}, self.round: {}",
                          header,
                          height,
                          self.round
                        );
                        ...
                        self.tx_process
                            .send(message)
                            .await
                            .expect("Failed to send new block to TxConversion");                            
