...
impl HeaderWaiter {
    ...
    async fn waiter(
        mut missing: Vec<(Vec<u8>, Store)>, // Vec<key, store>
        deliver: Header, 
        mut handler: Receiver<()>, //  for garbage collection
    ...
    async fn run(&mut self) {
        let mut waiting = FuturesUnordered::new(); // waiters
        ...
        loop {
            tokio::select! {
                Some(message) = self.rx_synchronizer.recv() => {
                    match message {
                        WaiterMessage::SyncBatches(missing, header) => { // missing: Hashmap<batch.digest, worker_id>
                            ...
                            for (worker_id, digests) in requires_sync {//request missing batch from header.author
                                let address = self.committee
                                    .worker(&author, &worker_id)// header.author
