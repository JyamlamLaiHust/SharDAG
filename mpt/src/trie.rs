...
pub struct MerklePatriciaTrie {
	root: Node,
	root_hash: Vec<u8>,
	pub db: Rc<MemoryDB>, // store all nodes in MPT
...
impl MerklePatriciaTrie {
	...
	fn get_at(&self, n: Node, partial: &Nibbles) -> TrieResult<Option<Vec<u8>>> {
		match n {
			Node::Empty => Ok(None),
			Node::Leaf(leaf) => {
				//let borrow_leaf = leaf.get_mut().unwrap();
	...
	fn insert_at(&self, n: Node, partial: Nibbles, value: Vec<u8>) -> TrieResult<Node> {
		match n {
			...
			Node::Leaf(leaf) => {
				// let mut borrow_leaf = leaf.get_mut().unwrap();
	...
	fn commit(&mut self) -> TrieResult<Vec<u8>> {
		...
		let root_hash = if encoded.len() < LENGTH {
			...
			self.cache.get_mut().unwrap().insert(hash.clone(), encoded);
			hash
		} else {
			encoded
		};

		let mut keys = Vec::with_capacity(self.cache.get_mut().unwrap().len());
		let mut values = Vec::with_capacity(self.cache.get_mut().unwrap().len());
		for (k, v) in self.cache.get_mut().unwrap().drain() {
			keys.push(k.to_vec());
			values.push(v);
		}
		...
		self.gen_keys.get_mut().unwrap().clear();
		self.passing_keys.get_mut().unwrap().clear();
