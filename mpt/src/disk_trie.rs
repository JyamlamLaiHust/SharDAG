...
#[derive(Debug)]
pub struct DiskMerklePatriciaTrie {
	root: Node,
	root_hash: Vec<u8>,
  pub db: DiskDB, // // store all nodes in kv store implemented on RocksDB
	cache: Mutex<HashMap<Vec<u8>, Vec<u8>>>,
	passing_keys: Mutex<HashSet<Vec<u8>>>,
	gen_keys: Mutex<HashSet<Vec<u8>>>,
}

impl DiskMerklePatriciaTrie {
  // create an empty MPT
  ...
  // recover the MPT given db and root
	pub async fn from(db: DiskDB, root: &[u8]) -> TrieResult<Self> {
...
impl Trie for DiskMerklePatriciaTrie {
...
impl DiskMerklePatriciaTrie {
	pub fn iter(&mut self) -> TrieIterator {
		let mut nodes = vec![];
...
pub struct TrieIterator<'a> {
	trie: &'a mut DiskMerklePatriciaTrie,
	nibble: Nibbles,
	nodes: Vec<TraceNode>,
}
