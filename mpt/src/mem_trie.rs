...
#[derive(Debug)]
pub struct MemMerklePatriciaTrie {
	root: Node,
	root_hash: Vec<u8>,
	pub db: Arc<MemoryDB>, // store all nodes in kv store implemented on HashMap
	cache: Mutex<HashMap<Vec<u8>, Vec<u8>>>,
	passing_keys: Mutex<HashSet<Vec<u8>>>,
	gen_keys: Mutex<HashSet<Vec<u8>>>,
}

impl MemMerklePatriciaTrie {
  // create an empty MPT
	pub fn new(db: Arc<MemoryDB>) -> Self {
		Self {
			...
			passing_keys: Mutex::new(HashSet::new()),
			gen_keys: Mutex::new(HashSet::new()),

			db,
		}
	}
  // recover the MPT given db and root
	pub async fn _from(db: Arc<MemoryDB>, root: &[u8]) -> TrieResult<Self> {
...
impl Trie for MemMerklePatriciaTrie {
...
impl MemMerklePatriciaTrie {
	pub fn _iter(&mut self) -> TrieIterator {
		let mut nodes = vec![];
	...
	fn get_at(&self, n: Node, partial: &Nibbles) -> TrieResult<Option<Vec<u8>>> {
		match n {
			Node::Empty => Ok(None),
			Node::Leaf(leaf) => {
				let borrow_leaf = leaf.lock().unwrap();
			...
			Node::Branch(branch) => {
				let borrow_branch = branch.lock().unwrap();
			...
			Node::Hash(hash_node) => {
				let borrow_hash_node = hash_node.lock().unwrap();
	...
	fn insert_at(&mut self, n: Node, partial: Nibbles, value: Vec<u8>) -> TrieResult<Node> {
		match n {
			...
			Node::Leaf(leaf) => {
				let mut borrow_leaf = leaf.lock().unwrap();
				...
				if match_index == 0 {
					return Ok(Node::Branch(Arc::new(Mutex::new(branch))));
				}

				// if include a common prefix
				Ok(Node::from_extension(
					partial.slice(0, match_index),
					Node::Branch(Arc::new(Mutex::new(branch))),
				))
			}
			Node::Branch(branch) => {
				let mut borrow_branch = branch.lock().unwrap();
			...
			Node::Extension(ext) => {
				let mut borrow_ext = ext.lock().unwrap();
				...
				if match_index == 0 {
					...
					let node = Node::Branch(Arc::new(Mutex::new(branch)));
	...
	fn delete_at(&mut self, n: Node, partial: &Nibbles) -> TrieResult<(Node, bool)> {
		let (new_n, deleted) = match n {
			...
			Node::Hash(hash_node) => {
				let hash = hash_node.lock().unwrap().hash.clone();
				self.passing_keys.lock().unwrap().insert(hash.clone());
	...
	fn degenerate(&mut self, n: Node) -> TrieResult<Node> {
		match n {
			...
			Node::Extension(ext) => {
				let borrow_ext = ext.lock().unwrap();

				let prefix = &borrow_ext.prefix;
				match borrow_ext.node.clone() {
					Node::Extension(sub_ext) => {
						let borrow_sub_ext = sub_ext.lock().unwrap();
	...
	fn get_path_at(&self, n: Node, partial: &Nibbles) -> TrieResult<Vec<Node>> {
		match n {
			...
			Node::Hash(hash_node) => {
				let n = self.recover_from_db(&hash_node.lock().unwrap().hash.clone())?;
	...
	pub fn commit(&mut self) -> TrieResult<Vec<u8>> {
		...
		let root_hash = if encoded.len() < LENGTH {
			...
			self.cache.lock().unwrap().insert(hash.clone(), encoded);
			hash
		} else {
			encoded
		};

		let mut keys = Vec::with_capacity(self.cache.lock().unwrap().len());
		let mut values = Vec::with_capacity(self.cache.lock().unwrap().len());
		for (k, v) in self.cache.lock().unwrap().drain() {
			keys.push(k.to_vec());
			values.push(v);
		}
		...
		let removed_keys: Vec<Vec<u8>> = self
			...
			.lock().unwrap()
			.iter()
			.filter(|h| !self.gen_keys.lock().unwrap().contains(&h.to_vec()))
			.map(|h| h.to_vec())
			.collect();
		...
		self.gen_keys.lock().unwrap().clear();
		self.passing_keys.lock().unwrap().clear();
	...
	fn encode_node(&self, n: Node) -> Vec<u8> {
		...
		if let Node::Hash(hash_node) = n {
			return hash_node.lock().unwrap().hash.clone();
		}
		...
		if data.len() < LENGTH {
			...
			self.cache.lock().unwrap().insert(hash.clone(), data);

			self.gen_keys.lock().unwrap().insert(hash.clone());
			hash
		}
	}
...
pub struct TrieIterator<'a> {
	trie: &'a mut MemMerklePatriciaTrie,
	nibble: Nibbles,
	nodes: Vec<TraceNode>,
}
			...
			if let Some(ref mut now) = now {
				...
				match (now.status.clone(), &now.node) {
					(TraceStatus::End, node) => {
						match *node {
							Node::Leaf(ref leaf) => {
								...
								self.nibble.truncate(cur_len - leaf.lock().unwrap().key.len());
							}

							Node::Extension(ref ext) => {
								let cur_len = self.nibble.len();
								self.nibble.truncate(cur_len - ext.lock().unwrap().prefix.len());
							}
					...
					(TraceStatus::Doing, Node::Extension(ref ext)) => {
						self.nibble.extend(&ext.lock().unwrap().prefix);
						self.nodes.push((ext.lock().unwrap().node.clone()).into());
					}

					(TraceStatus::Doing, Node::Leaf(ref leaf)) => {
						self.nibble.extend(&leaf.lock().unwrap().key);
						return Some((self.nibble.encode_raw().0, leaf.lock().unwrap().value.clone()));
					}

					(TraceStatus::Doing, Node::Branch(ref branch)) => {
						let value = branch.lock().unwrap().value.clone();
						if value.is_none() {
							continue;
						} else {
					...
					(TraceStatus::Doing, Node::Hash(ref hash_node)) => {
						if let Ok(n) = self.trie.recover_from_db(&hash_node.lock().unwrap().hash.clone()) {
							self.nodes.pop();
					...
					(TraceStatus::Child(i), Node::Branch(ref branch)) => {
						...
						self.nodes
							.push((branch.lock().unwrap().children[i as usize].clone()).into());
					}
...
mod tests {
    ...
    let memdb = Arc::new(MemoryDB::new());
		let mut trie = MemMerklePatriciaTrie::new(memdb);
	...
	fn test_trie_remove() {
    // Make the data store.
    let memdb: Arc<MemoryDB> = Arc::new(MemoryDB::new());
		let mut trie = MemMerklePatriciaTrie::new(memdb);
  ...
  //   // Make the data store.
  //   let store_path = "test_db";
  //   let diskdb = DiskDB::new(store_path);
	// 	let mut trie = DiskMerklePatriciaTrie::new(diskdb);
...
// 		let root = {
// 			let mut trie = DiskMerklePatriciaTrie::new(diskdb.clone());
// 			trie.insert(b"test".to_vec(), b"test".to_vec()).unwrap();
...
// 		let mut trie = DiskMerklePatriciaTrie::from(diskdb, &root).unwrap();
...
//       let store_path = "test_db";
//       let diskdb = DiskDB::new(store_path);
// 			let mut trie1 = DiskMerklePatriciaTrie::new(diskdb.clone());
...
// 			let mut trie2 = DiskMerklePatriciaTrie::from(diskdb, &root).unwrap();
...
// 		let mut trie = DiskMerklePatriciaTrie::from(diskdb, &root1).unwrap();
// 		trie.iter()
// 			.for_each(|(k, v)| assert_eq!(kv.remove(&k).unwrap(), v));
// 		assert!(kv.is_empty());
// 	}
